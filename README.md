...
1. Download appengine-go and dependencies by running: `goapp get -d github.com/nikmahes/appengine-go`. You may get messages about unrecognized imports. Ignore them.
1. `cd $GOPATH/src/github.com/nikmahes/appengine-go/app`.
1. Copy `app.sample.yaml` to `app.yaml`.
1. In the `appengine-go` directory (`cd ..` from the command above), copy `settings.go.dist` to `settings.go`.
